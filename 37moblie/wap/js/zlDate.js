...
var pickerEvent = {
    Init: function (elemid) {
        ...
        document.getElementById("calendar_choose").style.zIndex = 1000111;
        ...
        for (var i = 0; i < tds.length; i++) {
            if (tds[i].getAttribute("date") != null && tds[i].getAttribute("date") != "" && tds[i].getAttribute("price") != "-1") {
                tds[i].onclick = function () {
					...
					document.getElementById("TimeTan").style.display="none";
...
var pickerHtml = {
    ...
    getLeft: function () {
        var left = '<div class="calendar_left pkg_double_month"><p class="date_text">' + obj.year + '年' + obj.month + '月</p><a href="javascript:void()" title="上一月" id="picker_last" class="pkg_circle_top">上一月</a><a href="javascript:void()" title="下一月" id="picker_next" class="pkg_circle_bottom ">下一月</a></div>';
        htmlObj.left = left;
    },
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
						if (obj.month < 10 &&obj.day < 9)
				        {
							html += '<td  ' + classStyle + ' date="' + obj.year + "-"+"0" + obj.month + "-"+"0" + (i - c) + '" price="' + price + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				        }else if(obj.month < 10&&obj.day >= 9){
							html += '<td  ' + classStyle + ' date="' + obj.year + "-"+"0" + obj.month + "-" + (i - c) + '" price="' + price + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';	
				        }else if(obj.month > 10&&obj.day <= 9){
							html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-"+"0" + (i - c) + '" price="' + price + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';	
				        }else if(obj.month > 10&&obj.day >= 9){
						...
						if (obj.month < 10 &&obj.day < 9)
				        {
                			html += '<td  ' + classStyle + ' date="' + obj.year + "-"+"0" + obj.month + "-"+"0" + (i - c) + '" price="' + price + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
							
				        }else if(obj.month < 10&&obj.day >= 9){
				        	
							html += '<td  ' + classStyle + ' date="' + obj.year + "-"+"0" + obj.month + "-" + (i - c) + '" price="' + price + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';	
				        }else if(obj.month > 10&&obj.day <= 9){

							html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-"+"0" + (i - c) + '" price="' + price + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';	
				        }else if(obj.month > 10&&obj.day >= 9){
...
var commonUtil = {
    ...
    chooseClick: function (sender) {
    	//fjl 20170209 add
    	busGoodsInit(0);//点击日期获取 日期对应门票金额
    	//fjl 20170209 end
